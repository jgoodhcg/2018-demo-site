...
The `home-routes` handler in the `demo-site.routes.home` namespace
...
The `home-page` function will in turn call the `demo-site.layout/render` function
...
The rest of this page is rendered by ClojureScript found in the `src/cljs/demo_site/core.cljs` file.



#### Organizing the routes

The routes are aggregated and wrapped with middleware in the `demo-site.handler` namespace:
...
Request middleware functions are located under the `demo-site.middleware` namespace.

This namespace is reserved for any custom middleware for the application. Some default middleware is
already defined here. The middleware is assembled in the `wrap-base` function.

Middleware used for development is placed in the `demo-site.dev-middleware` namespace found in
...
* Let `mount` know to start the database connection by `require`-ing `demo-site.db.core` in some other namespace.
